...
class BilibiliGUI():
    def __init__(self) -> None:
        ...
        self.layouttop = [[sg.Submit('启动互助', key="-Ok-"), sg.Button('退出互助', key="Quit"), sg.Text(size=(10), key='-OUTPUT-'), ]
                        ]
        self.layoutr = [[sg.Text("请输入BV号,一行一个(最大支持20)")],
                        [sg.Multiline('BV1CW4y1h7U2\nBV1n14y1b7yw', size=(20,18), key='-bvlist-')]
                        ]
        self.layoutl = [[sg.Text("观看"), sg.Checkbox('   ', key='-watch-', default=True), sg.Text("点赞"), sg.Checkbox('', key='-like-')],
                        [sg.Text("投币"), sg.Checkbox('   ', key='-reward-'), sg.Text("双倍"), sg.Checkbox('', key='-reward2-')],
                        [sg.Text("收藏"), sg.Checkbox('   ', key='-favour-'), sg.Text("分享"), sg.Checkbox('', key='-share-')],
                        [sg.Text("弹幕"), sg.Checkbox('   ', key='-danmaku_post-'), sg.Text("三连"), sg.Checkbox('', key='-combo-')],
                        [sg.Text("弹幕内容(该功能敬请期待):")],
    ...
    def visit_vip_aid(self, cookie, vip_aid_list):
        ...
        for vip_aid in vip_aid_list:
            dic = {"aid_list": [vip_aid],  # aid列表
                    ...
                    "like_enable": True,  # 点赞开关
                    "reward_enable": False,  # 投币开关
                    "reward2_enable": False,  # 投币双倍开关
                    "favour_enable": True,  # 收藏开关
                    "combo_enable": True,  # 一键三连开关
                    "share_enable": True,  # 分享开关
                    "danmaku_post_enable": True,  # 弹幕开关
    ...
    def run(self, vis_chrome=False):
        ...
        while True:
            ...
            if len(cookie) < 2 or cookie == None:
                window['-OUTPUT-'].update("请重新登录.")
                return
            if self.event == "-Ok-":  # 互助开启
                sg.popup_ok("点击OK 程序后台将自动互助\n无法点击程序是正常情况, 挂机即可\n", title="互助提醒")
                upload_dic = self.transforGuiInput(cookie)  # 转换输入数据为上传dict
                online_return_dic = pull_bv_list(upload_dic)  # 拉取数据库数据 获取bvlist

                if online_return_dic["expired"]:
                    window['-OUTPUT-'].update(f"会员已过期: {online_return_dic['duedate']}")
                else:
                    window['-OUTPUT-'].update(f"会员互助已启动: {online_return_dic['duedate']}")
